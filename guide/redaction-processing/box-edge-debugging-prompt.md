...
`python -m guesser_core.logic.debug.debug_width`
([guesser_core/logic/debug/debug_width.py](../../guesser_core/logic/debug/debug_width.py))
reproduces the real refiner on any page and bundles everything below into one
command. It loads the page through the **same** `ProcessRedactions.extract_page_image`
...
```bash
python -m guesser_core.logic.debug.debug_width --page 8            # ground-truth case
python -m guesser_core.logic.debug.debug_width --page 8 --pixels   # + ASCII map & column stats (scripts B/C)
python -m guesser_core.logic.debug.debug_width --pdf <path> --page 3 --box 0
python -m guesser_core.logic.debug.debug_width --verify            # regression check (script in "Verify" below)
```
...
1. [guesser_core/logic/BoxDetector.py](../../guesser_core/logic/BoxDetector.py)
...
2. [guesser_core/logic/ProcessRedactions.py](../../guesser_core/logic/ProcessRedactions.py)
...
3. [guesser_core/logic/refiners/pipeline.py](../../guesser_core/logic/refiners/pipeline.py)
...
- Line text: `EPSTEIN, including ░░░░░░░░░ and GHISLAINE MAXWELL.`
...
Standard image load (matches ProcessRedactions' crop) used by all scripts:
...
> Note: `etv_refiner.py` imports `guesser_core.logic.SurroundingWordWidth`, so run
> scripts from the repo root with `sys.path.insert(0, os.getcwd())` (not from
> inside `guesser_core/logic`).
...
There are **no unit tests** in `guesser_core/tests.py`; verify by reproducing on

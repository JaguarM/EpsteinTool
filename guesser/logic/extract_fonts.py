...
def detect_dominant_font(text_spans, available_fonts):
    """
    Detect the dominant font from a list of text spans.
    ...
    available_fonts : list of str
        .ttf filenames available on this server (from width_calculator.get_available_fonts).

    Returns
    -------
    dict with keys:
        font_file  : str | None  – e.g. "times.ttf", or None if no match found
        font_size  : float       – dominant size in points
    ...
    # Dominant size = the size with the most characters for that font
    ...
    return {
        ...
        "pdf_font_name": dominant_pdf_font,
    }
